...
const Home: NextPage = () => {
  ...
  // Auto-execute testSignTransaction for Tron after successful connection
  useEffect(() => {
    if (session && accounts.length > 0) {
      // Find Tron account
      const tronAccount = accounts.find((account) =>
        account.startsWith("tron:")
      );

      if (tronAccount) {
        const [namespace, reference, address] = tronAccount.split(":");
        const chainId = `${namespace}:${reference}`;

        // Auto-execute testSignTransaction after a short delay
        const timer = setTimeout(async () => {
          try {
            console.log("Auto-executing testSignTransaction for Tron...");
            await tronRpc.testSignTransaction(chainId, address);
          } catch (error) {
            console.error(
              "Auto-execution of testSignTransaction failed:",
              error
            );
          }
